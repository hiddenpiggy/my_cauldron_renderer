...
void Texture::setContext(VkContext *context) { this->context = context; }

void Texture::setCommandBuffers(VkCommandBuffers *commandBuffers) {
  this->commandBuffers = commandBuffers;
}

void Texture::setAllocator(VmaAllocator allocator) {
  this->allocator = allocator;
}

void Texture::setBufferPool(BufferPool *pBufferPool) {
...
void Texture::destroy() {
  ...
  ImageWrapper wrapper = {image, allocation, allocationInfo};
  bufferPool->freeImage(wrapper);
  device.destroySampler(sampler);
}

void Texture::fromglTFImage(tinygltf::Image &gltfimage,
                            TextureSampler textureSampler, vk::Queue copyQueue,
  ...
  //   &memReqs); memAllocInfo.allocationSize = memReqs.size;
  ...
  imageAllocCreateInfo.flags = VMA_ALLOCATION_CREATE_MAPPED_BIT;
  imageAllocCreateInfo.usage = VMA_MEMORY_USAGE_GPU_ONLY;
  ImageWrapper imageWrapper =
      bufferPool->allocateMeomryForImage(imageCreateInfo, imageAllocCreateInfo);

  image = imageWrapper.image;
  allocation = imageWrapper.allocation;
  allocationInfo = imageWrapper.allocationInfo;
  ...
  resourceUploadHeap->transitionImageLayout(
      image, format, vk::ImageLayout::eUndefined,
      vk::ImageLayout::eTransferDstOptimal);
  resourceUploadHeap->uploadImageData(buffer, bufferSize, width, height, image,
                                      imageWrapper.allocation);
  resourceUploadHeap->transitionImageLayout(
  ...
  for (uint32_t i = 1; i < mipLevels; i++) {
    ...
    vk::CommandBuffer blitCmd = commandBuffers->beginSingleTimeCommands();
    {
    ...
    commandBuffers->endSingleTimeCommands(blitCmd);
  }
  ...
  //vk::ArrayProxy<vk::CommandBuffer> blitCmds{blitCmd};
  //device.freeCommandBuffers(commandBuffers->getCommandPool(), blitCmds);

  VkSamplerCreateInfo samplerInfo{};
  samplerInfo.sType = VK_STRUCTURE_TYPE_SAMPLER_CREATE_INFO;
  samplerInfo.magFilter = static_cast<VkFilter>(textureSampler.magFilter);
  samplerInfo.minFilter = static_cast<VkFilter>(textureSampler.minFilter);
  samplerInfo.mipmapMode = VK_SAMPLER_MIPMAP_MODE_LINEAR;
  samplerInfo.addressModeU =
      static_cast<VkSamplerAddressMode>(textureSampler.addressModeU);
  samplerInfo.addressModeV =
      static_cast<VkSamplerAddressMode>(textureSampler.addressModeV);
  samplerInfo.addressModeW =
      static_cast<VkSamplerAddressMode>(textureSampler.addressModeW);
...
// Mesh
glTFMesh::glTFMesh(VkContext *context, BufferPool *bufferPool,
                   glm::mat4 matrix) {
  ...
  auto queueFamilyIndex =
      context->getQueueFamilyIndices().graphicsFamilyIndex.value();

  // create uniform buffer
  vk::BufferCreateInfo bufferCreateInfo{
      {},                   // flags
      sizeof(uniformBlock), // size
  ...
  BufferWrapper bufferwrapper =
      bufferPool->allocateMemory(bufferCreateInfo, allocationCreateInfo);
  uniformBuffer.buffer = bufferwrapper.buffer;
  uniformBuffer.memory = bufferwrapper.allocationInfo.deviceMemory;
  uniformBuffer.mapped = bufferwrapper.allocationInfo.pMappedData;

  // set necessary handles
  allocation = bufferwrapper.allocation;
  allocationInfo = bufferwrapper.allocationInfo;
  uniformBuffer.descriptor = {uniformBuffer.buffer, 0, sizeof(uniformBlock)};
}

glTFMesh::~glTFMesh() {
  BufferWrapper bufferWrapper{uniformBuffer.buffer, allocation, allocationInfo};
  bufferPool->freeBuffer(bufferWrapper);
...
// Model
void glTFModel::destroy(vk::Device device) {
  if (vertices.buffer != VK_NULL_HANDLE) {
    BufferWrapper bufferWrapper = { vertices.buffer, vertices.bufferAllocation, vertices.bufferAllocationInfo};
    bufferPool->freeBuffer(bufferWrapper);
  ...
  if (indices.buffer != VK_NULL_HANDLE) {
    BufferWrapper bufferWrapper = { indices.buffer, indices.bufferAllocation, indices.bufferAllocationInfo };
    bufferPool->freeBuffer(bufferWrapper);
...
void glTFModel::loadNode(Node *parent, const tinygltf::Node &node,
  ...
  if (node.mesh > -1) {
    ...
    glTFMesh *newMesh = new glTFMesh(context, bufferPool, newNode->matrix);
...
void glTFModel::getNodeProps(const tinygltf::Node &node,
...
void glTFModel::loadTextures(tinygltf::Model &gltfModel, VkContext *context,
  ...
  for (tinygltf::Texture &tex : gltfModel.textures) {
    ...
    if (tex.sampler == -1) {
      ...
      textureSampler.magFilter = static_cast<vk::Filter>(VK_FILTER_LINEAR);
      textureSampler.minFilter = static_cast<vk::Filter>(VK_FILTER_LINEAR);
      textureSampler.addressModeU =
          static_cast<vk::SamplerAddressMode>(VK_SAMPLER_ADDRESS_MODE_REPEAT);
      textureSampler.addressModeV =
          static_cast<vk::SamplerAddressMode>(VK_SAMPLER_ADDRESS_MODE_REPEAT);
      textureSampler.addressModeW =
          static_cast<vk::SamplerAddressMode>(VK_SAMPLER_ADDRESS_MODE_REPEAT);
    } else {
    ...
    texture.setCommandBuffers(commandBuffers);
    texture.setBufferPool(bufferPool);
    VmaAllocator allocator = bufferPool->getAllocator();
    texture.setAllocator(allocator);
    texture.fromglTFImage(image, textureSampler, transferQueue,
                          resourceUploadHeap);
...
vk::SamplerAddressMode glTFModel::getVkWrapMode(int32_t wrapMode) {
  switch (wrapMode) {
  case -1:
  case 10497:
...
vk::Filter glTFModel::getVkFilterMode(int32_t filterMode) {
  switch (filterMode) {
  case -1:
  case 9728:
...
void glTFModel::loadFromFile(std::string filename, BufferPool *bufferPool,
                             VkContext *context,
                             ResourceUploadHeap *resourceUploadHeap,
                             VkCommandBuffers *commandBuffers,
                             VkQueue transferQueue, float scale) {
  vk::Device device = context->getDevice();
  this->bufferPool = bufferPool;
  this->context = context;
  this->commandBuffers = commandBuffers;
  this->resourceUploadHeap = resourceUploadHeap;
  ...
  if (fileLoaded) {
    ...
    loadTextures(gltfModel, context, commandBuffers, bufferPool,
                 resourceUploadHeap, transferQueue);
  ...
  BufferWrapper vertexBufferWrapper =
      bufferPool->allocateMemory(vertexBufferCreateInfo, allocCreateInfo);
  vertices.buffer = vertexBufferWrapper.buffer;
  vertices.memory = vertexBufferWrapper.allocationInfo.deviceMemory;
  vertices.bufferAllocation = vertexBufferWrapper.allocation;
  vertices.bufferAllocationInfo = vertexBufferWrapper.allocationInfo;
  // Upload buffer data
  resourceUploadHeap->uploadBufferData(
      loaderInfo.vertexBuffer, vertexBufferSize, vertexBufferWrapper.buffer,
      vertexBufferWrapper.allocation, 0);

  // Upload index buffer data
  if (indexBufferSize > 0) {
    BufferWrapper indexBufferWrapper =
        bufferPool->allocateMemory(indexBufferCreateInfo, allocCreateInfo);
    
    indices.buffer = indexBufferWrapper.buffer;
    indices.memory = indexBufferWrapper.allocationInfo.deviceMemory;
    indices.bufferAllocation = indexBufferWrapper.allocation;
    indices.bufferAllocationInfo = indexBufferWrapper.allocationInfo;

    resourceUploadHeap->uploadBufferData(
        loaderInfo.indexBuffer, indexBufferSize, indexBufferWrapper.buffer,
        indexBufferWrapper.allocation, 0);
  }

  // clean unnecessary buffers
  delete[] loaderInfo.vertexBuffer;
  delete[] loaderInfo.indexBuffer;
  getSceneDimensions();
}
...
void glTFModel::draw(vk::CommandBuffer commandBuffer) {
